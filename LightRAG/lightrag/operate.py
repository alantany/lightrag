...
async def naive_query(
    ...
    try:
        logger.info(f"Starting naive query: {query}")
        
        results = await chunks_vdb.query(query, top_k=param.top_k)
        logger.info(f"VDB query results: {results}")
        
        if not results:
            logger.warning("No results from VDB query")
            return "未找到相关信息。"
        
        content_data = await text_chunks.get_by_ids([r["id"] for r in results])
        ...
        content_data = [c["content"] for c in content_data if c]
        content_data = "\n".join(content_data)
        ...
        logger.exception(f"Error in naive_query: {str(e)}")
        raise
